...
class OpenlaneDataset(Dataset):
    def __init__(self, 
        ...
        self.img_dir = os.path.join(data_root, 'images/validation')
        ...
        self.x_min = x_min  # -10
        self.x_max = x_max  # 10
        self.anchor_y_steps = np.array(y_steps, dtype=np.float64)
    ...
    def eval(self, pred_filename, eval_file, prob_th=0.5):
        ...
        json_gt = [json.loads(line) for line in open(eval_file).readlines()]
        ...
        gts = {'segment'+l['file_path'].split('segment')[1]: l for l in json_gt}
