    ...
    def _resize_img(self, results):
        ...
        if self.keep_ratio:
            ...
            if len(results['img'].shape) > 3:
                img = [mmcv.imrescale(
                results['img'][..., idx], results['scale'], return_scale=True)[0] for idx in range(results['img'].shape[-1])]
            ...
            if len(results['img'].shape) > 3:
                img_with_scale = [mmcv.imresize(
                results['img'][..., idx], results['scale'], return_scale=True) for idx in range(results['img'].shape[-1])]
                img = [im[0] for im in img_with_scale]
                img = np.stack(img, axis=-1)
                w_scale = img_with_scale[0][1]
                h_scale = img_with_scale[0][2]
            else:
