...
def extract_data_with_smoothing(data_root, anno_file, tar_path, max_lanes=20, test_mode=False, sample_step=1, prune_vis=True, smooth=True):
    ...
    for image_id, old_anno in old_annotations.items():
        ...
        pickle_path = os.path.join(tar_path, '/'.join(anno['filename'].split('/')[-2:-1]))
        mkdir_or_exist(pickle_path)
...
if __name__ == '__main__':
    ...
    if args.merge:
        name = args.name
        merge_annotations(os.path.join(args.data_root, name), os.path.join(args.data_root, f'{name}.json'))
        print("Finished merge Process")
    elif args.generate:
        name = args.name
        ori_json = os.path.join(args.data_root, f'{name}.json')
        ...
        extract_data_with_smoothing(args.data_root, ori_json, tar_path=tar_path, test_mode=args.testMode)
        generate_datalist(os.path.join(tar_path), os.path.join(data_list_path, f'{name}.txt'))