...
class Anchor3DLane(BaseModule):
    ...
    def get_proposals(self, project_matrixes, anchor_feat, iter_idx=0, proposals_prev=None):
        project_matrixes = project_matrixes.float()
        anchor_feat = anchor_feat.float()
        if proposals_prev is not None:
            proposals_prev = proposals_prev.float()
        ...
        if proposals_prev is None:
            ...
            sampled_anchor = torch.zeros(batch_size, len(self.anchors), 5 + self.anchor_feat_len * 3, device=anchor_feat.device).float()
        ...
        batch_anchor_features = batch_anchor_features.transpose(1, 2).float()  # [B, N, C, l]
        ...
        if proposals_prev is None:
            reg_proposals[:, :, :5+self.anchor_len*3] = reg_proposals[:, :, :5+self.anchor_len*3] + self.anchors.float()
        else:
            reg_proposals[:, :, :5+self.anchor_len*3] = reg_proposals[:, :, :5+self.anchor_len*3] + proposals_prev[:, :, :5+self.anchor_len*3].float()
    ...
    def loss(self, output, gt_3dlanes, output_aux=None):
        ...
        for proposal, anchor in zip(output['reg_proposals'], output['anchors']):
            proposals_list.append((proposal.float(), anchor.float()))
        ...
        for iter in range(self.iter_reg):
            ...
            for proposal, anchor in zip(output_aux['reg_proposals'][iter], output_aux['anchors'][iter]):
                proposals_list_aux.append((proposal.float(), anchor.float()))
            anchor_losses_aux = self.aux_lossiter
